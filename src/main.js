...
function bootstrap() {
  ...
  app.innerHTML = `
    ...
    <section>
      ...
      <p>Place your file at <code>public/blockchain-chucky.svg</code> to render it on GitHub Pages.</p>
      <img id="featuredImage" class="featuredImage" alt="Blockchain Chucky" src="${import.meta.env.BASE_URL}blockchain-chucky.svg" />
      <p id="featuredImageStatus" class="hint"></p>
    </section>
  ...
  const featuredImageStatus = document.querySelector("#featuredImageStatus");
  featuredImage.onload = () => {
    featuredImageStatus.textContent = `Image loaded from ${featuredImage.getAttribute("src")}`;
  };
  featuredImage.onerror = () => {
    featuredImageStatus.textContent = "Image not found. Add file at public/blockchain-chucky.svg and redeploy.";
    featuredImage.classList.add("missing");
  };
  ...
  document.querySelector("#register").onclick = withGuard(async () => {
    const [{ uploadMedia, uploadJson }] = await Promise.all([import("./irys")]);
    ...
    if (!wallet?.provider || !wallet?.signer) throw new Error("Connect wallet first");
    if (!cover || !audio) throw new Error("cover and audio are required");

    const coverUpload = await uploadMedia(wallet.provider, cover, ["image/"]);
    const audioUpload = await uploadMedia(wallet.provider, audio, ["audio/", "video/"]);
    const metadata = buildMetadata({
      title,
      artist: wallet.address,
      description,
      coverUri: coverUpload.uri,
      audioUri: audioUpload.uri,
      coverMime: coverUpload.mime,
      audioMime: audioUpload.mime
    });
    const coverUri = coverUpload.uri;
    const audioUri = audioUpload.uri;
...
try {
  ...
  const app = document.querySelector("#app");
  if (app) {
    app.innerHTML = `<section><h2>App failed to initialize</h2><p>${error?.message || error}</p></section>`;
  }
  console.error(error);
}
